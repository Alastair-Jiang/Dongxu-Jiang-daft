...
- **问题**: 200 维 s_t 的 6 个特征组均用 while 循环反复追加同一表达式
  补齐到固定宽度（K3 纲领 `docs/K3_GUIDANCE_2026-08-18.md` A5）。
  200 维中 70 维为纯复制列，另有 8 处跨组/组内同值重复（G1 累计收益与
  多尺度波动 6 列、G2 体量秩 w=1、G3 裸 vol_20、G4 裸 spread、G5 两处
  秩重复、G6 动量三列）——有效维度仅约 128，虚假放大共线性；
  "6 组 45/35/40/20/30/30"的声明与实际不符，特征组消融（g2/g3 负贡献）
  的归因解释力受损。
- **修复**: `features/regime_features.py` 改为命名注册表——每个特征
  具名（`feature_names` 200 项两两不同），新增 `_pad_and_stack`
  （组内同名去重 + 不足槽位零填充并命名 g{k}_pad_XX）与
  `real_feature_mask`（True=真实特征）/ `n_real_features` /
  `n_padding` / `group_real_counts` 诊断出口。日频真实特征 115 个
  （g1:27 / g2:20 / g3:20 / g4:14 / g5:15 / g6:19）+ 85 个显式零填充列；
  全部 while 复制列与 8 处同值重复列删除；顺手修复 G4 脆弱索引
  `feats[-2]`。周线 `lookback_scale=0.2` 窗口塌缩产生的同名列按
  注册表自动去重（首现保留）。下游 200 维契约（输出形状与组槽位宽度）
  不变。
- **守卫**: `tests/test_features.py::TestA5FeatureRegistry`（9 项：
  while 填充回归守卫/注册表完整性/名称唯一性/日频计数精确断言/
  填充列恒零/填充显式命名/真实列信息量/注册表幂等/周线去重）+
  `self_check.py` 5.7 节 4 项断言（while 清零/注册表齐备/填充命名/
  历史重复表达式清除）。
- **影响面**: 纯特征层重构——删除的重复列不携带增量信息，全部真实特征
  表达式逐字保留（git diff 全量核对）。填充列恒 0，不再放大共线性；
  router/专家的输入分布改变（原复制列消失），历史训练产物与修复后
  数值不可直接比较。按纲领要求：特征组消融（g2/g3 负贡献结论）需在
  去重后同口径重跑复核（EXP 产物待补，可与 A4 的对决重跑合并执行）。

### B1 — v0.2.1 偏差审计纲领 + B 类补丁系列立项（2026-08-19）

- **问题**: v0.2.1 时部分训练未按计划完成（transformer 扩规模批次波1/波2
  三任务悬空：tf100_128x4 暂停于 Stage3 后、tf300_128x4 暂停于 Stage1 中途、
  tf100_256x8 未开始，判定路径未收口）；部分实验数据算完即写入登记文档，
  未经独立复核——全部产出自 A1-A5 修复前的代码，作为后续优化与数学公式
  搭建的参照系不保证正确（锚点 EXP-20260817-51 与阈值 0.0331/0.0482 均待重算重声明）。
- **补丁**: 新增 `docs/B_PATCH_GUIDANCE_2026-08-19.md`——B 类补丁系列总纲：
  ① 命名规范 `patch(B{n})` 系列与验收门槛（沿用 A 系列流水线）；
  ② 偏差主表 D1-D8（计划 vs 实际出入清单）；③ 受影响数字资产清单与
  正确性影响分级（稳/疑/废）；④ 补丁路线 B2-B15（B2/B3 文档口径标注与
  计划状态同步立即执行；B4 锚点重算与 B5 transformer 收口需 GPU，时机由
  维护者定；B6-B15 映射接管 K3 级 B1-B10 工程质量项）。
- **守卫**: `self_check.py` 5.8 节 2 项断言（纲领在库/审计表锚点齐备）。
  验收中顺带清除 CHANGELOG 残留的 81 处数字转义实体（A5 条目旧污染，
  当时实体检查正则未覆盖数字实体）。
- **影响面**: 纯文档 + 自检断言，不动训练/评估数学。生效后: 旧数字引用面
  封存待 B2 标注，transformer 线恢复以前置 B4 锚点重算为硬门槛。
